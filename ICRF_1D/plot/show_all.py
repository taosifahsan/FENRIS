"""Render every ICRF_1D figure and movie from one snapshot pass.

The 1-D version of ``ICRF_2D/plot/show_all.py``: everything lands in a fresh
...
Outputs: solution, temperature, grid_level, grid_dof, growth_average (PNG);
solution, temperature, grid_level, growth_rate (MP4).
...
def main():
    parser = argparse.ArgumentParser(description="ICRF_1D: all figures + movies")
    ...
    if do_static:
        ...
        save_png(solution.plot_temperature_static(solution_data), out,
                 "temperature", dpi=220)
    ...
    if do_movie:
        solution.plot_movie(
            ...
            workers=args.workers, fps=args.fps, dpi=args.dpi)
        solution.plot_temperature_movie(
            solution_data, str(Path(out) / "temperature.mp4"),
            workers=args.workers, fps=args.fps, dpi=args.dpi)
