"""ICRF_1D's one figure: distribution + effective temperature, two panels.

The 1-D isotropic speed problem has an analytic steady state, so every frame
compares the solver's state against it directly, as two separate figures
(each with its own movie):

  solution     -- f(v) versus the analytic steady state
  temperature  -- the local effective temperature T(v) = -f dE/df of both,
                  with the flat T_eff prediction and the large-v ~ v^3 slope
...
def draw_solution_frame(fig, ax, data, index):
    ...
    draw = getattr(ax, data["scale_dist"], ax.plot)
...
def draw_temperature_frame(fig, ax, data, index):
    ...
    time_label = rf"$(v/v_{{th}},\ t/\tau = {data['times'][index]:.2f})$"
    draw = getattr(ax, data["scale_temp"], ax.plot)
...
def plot_static(data):
    """Final-snapshot distribution figure."""
    fig, ax = plt.subplots(figsize=(7.2, 5.2))
    draw_solution_frame(fig, ax, data, len(data["frames"]) - 1)
    return fig


def plot_temperature_static(data):
    """Final-snapshot temperature figure."""
    fig, ax = plt.subplots(figsize=(7.2, 5.2))
    draw_temperature_frame(fig, ax, data, len(data["frames"]) - 1)
    return fig


# Per-worker state for the movies, sent once via the pool initializer.
_DATA = None
...
def _draw_solution_frame_task(task):
    """Worker: one distribution frame (fixed dims for H.264)."""
    index = task["index"]
    fig, ax = plt.subplots(figsize=(7.2, 5.2))
    draw_solution_frame(fig, ax, _DATA, index)
    fig.savefig(f"{task['frame_dir']}/frame_{index:06d}.png", dpi=task["dpi"])
    plt.close(fig)


def _draw_temperature_frame_task(task):
    """Worker: one temperature frame (fixed dims for H.264)."""
    index = task["index"]
    fig, ax = plt.subplots(figsize=(7.2, 5.2))
    draw_temperature_frame(fig, ax, _DATA, index)
    fig.savefig(f"{task['frame_dir']}/frame_{index:06d}.png", dpi=task["dpi"])
    plt.close(fig)


def plot_movie(data, output_file, *, workers=None, fps=8, dpi=140):
    """Render the distribution movie, one frame per snapshot."""
    return render_movie(
...
def plot_temperature_movie(data, output_file, *, workers=None, fps=8, dpi=140):
    """Render the temperature movie, one frame per snapshot."""
    return render_movie(
        _draw_temperature_frame_task, len(data["frames"]), output_file,
        fps=fps, dpi=dpi, workers=workers,
        initializer=_init_solution_worker, initargs=(data,),
    )
...
def main():
    ...
    if do_static:
        ...
        save_png(plot_temperature_static(data), args.fig_dir, "temperature",
                 dpi=220)
    if do_movie:
        plot_movie(data, str(Path(args.fig_dir) / "solution.mp4"),
                   workers=args.workers, fps=args.fps, dpi=args.dpi)
        plot_temperature_movie(
            data, str(Path(args.fig_dir) / "temperature.mp4"),
            workers=args.workers, fps=args.fps, dpi=args.dpi)
