"""Plot everything for ICRF_2D, in one run.
...
1. **No-snapshot figures first** (``coefficients.py``, ``flux_surface.py``) --
   these need only the deck and the coefficient tables, so they run
   concurrently with nothing to wait for.
...
def main():
    parser = argparse.ArgumentParser(description="Plot everything for ICRF_2D")
    ...
    #        to wait for -----------------------------------------------------
    no_snapshot_jobs = [
        ("ql_coefficients", coefficients.plot_coefficients, ()),
        ("collision_coefficients", coefficients.plot_collision_coefficients, ()),
        ("li_factors", coefficients.plot_li, ()),
        ("trajectory", flux_surface.plot_trajectory, ()),
    ]
    if do_static:
    ...
    save_input_files(output_dir, PATHS.solver_input, PATHS.build_input)
