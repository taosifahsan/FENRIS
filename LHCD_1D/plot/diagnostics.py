...
The 1-D version of the growth pieces of ``LHCD_2D/plot/diagnostics.py``:
...
def _moments(cache, solver_input=None):
    """Number and energy histories under the parallel-velocity measure.

    N(t) = int f dv;  E(t) = int (v^2/2) f dv (normalized units, mass 1).
    """
    del solver_input
    ...
    for frame in cache.frames:
        ...
        number.append(float(np.sum(f) * dv))
        energy.append(float(0.5 * np.sum(v**2 * f) * dv))
